...
check whether the internet is down or not. You are free to add changes.This script dosnt use the icmp protocol which is a little bit complex to undrstand when just getting started.The script also stores the result in a file in a csv format named log.csv. The script will create the log file if it is not present on the system and if it present then it will simple keep logging in it.
</p>
